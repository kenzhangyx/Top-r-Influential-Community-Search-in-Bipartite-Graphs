...
double UpperBound(BiGraph& g)
{
    int n=g.num_v1+g.num_v2;
    vector<int> u;
    u.push_back(-1);
    vector<int> x[n+1];
    vector<int> y[n+1];
    for (auto i:g.v1) {y[0].push_back(i);u.push_back(i);}
    for (auto i:g.v2) {y[0].push_back(i);u.push_back(i);}
    double a[n+1];
    double b[n+1];
    ...
    for (int i=1;i<=n;i++)
    {
        auto tmpx=x[i-1];
        tmpx.push_back(u[i]);
        a[i]=avg(tmpx)-avg(x[i-1]);
        auto tmpy = y[i-1];
        for(int j=0;j<tmpy.size();j++)
        {
            if (tmpy[j] == u[i])
            {
                tmpy.erase(tmpy.begin()+j);
                break;
            }
        }
        b[i]=avg(tmpy)-avg(y[i-1]);
...
{ 
    ...
    for (auto subg:setlist)
    {
        ...
        if (visit1[i] == false)
        {
            ...
            auto abtmp = findabcore(tmp);
            for (auto j:tmp.v1)
            {
                if (existU(j,abtmp) == false) visit1[j] = true;
            }
            for (auto j:tmp.v2)
            {
                if (existV(j,abtmp) == false) visit2[j] = true;
            }
            if (UpperBound(abtmp) >= compute_avg(topr[0]))
            {
            Find(abtmp);
            }
        }
