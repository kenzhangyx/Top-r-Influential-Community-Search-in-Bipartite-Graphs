    ...
    for (auto cg:setlist)
    {
        for (auto i:cg.v1)
        {
            ...
            while(q1.empty()==false || q2.empty()==false)
            {
                ...
                {
                    //sort(neighbor.begin(),neighbor.end(),desc);
                    for (auto k=0;k<Alpha;k++)
                    {
                        if (vecfind(nodeset[1],cg.getV1Neighbors(node)[k]) == false) 
                        {
                            visit2[cg.getV1Neighbors(node)[k]] = true;
                            nodeset[1].push_back(cg.getV1Neighbors(node)[k]);
                            q2.push(cg.getV1Neighbors(node)[k]);
                        }
                    }
                ...
                else //select form upper set
                {
                    ...
                    //sort(neighbor.begin(),neighbor.end(),desc);
                    for (auto k=0;k<Beta;k++)
                    {
                        if (vecfind(nodeset[0],cg.getV2Neighbors(node)[k]) == false) 
                        {
                            visit1[cg.getV2Neighbors(node)[k]] = true;
                            nodeset[0].push_back(cg.getV2Neighbors(node)[k]);
                            q1.push(cg.getV2Neighbors(node)[k]);
                        }
                    }
                }
            }
